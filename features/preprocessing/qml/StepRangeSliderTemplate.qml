...
Item {
    ...
    height: contentColumn ? contentColumn.implicitHeight : 80
        ...
        Column {
            ...
            Row {
                ...
                TextField {
                    ...
                    validator: DoubleValidator { bottom: 0.000001; top: 1000000 }
                    onAccepted: updateStepSize()
                    Component.onCompleted: text = rangeSliderTemplate.formatValue(rangeSliderTemplate.stepSize)
                    onActiveFocusChanged: {
                        if (!activeFocus) {
                            text = rangeSliderTemplate.formatValue(rangeSliderTemplate.stepSize)
                        }
                    }
                }
            }
    ...
    function updateStepSize() {
        var newStepSize = parseFloat(stepSizeInput.text)
        if (isNaN(newStepSize) || newStepSize <= 0) {
            warningText.text = "Invalid step size (must be > 0)"
            return
        }
        stepSize = newStepSize
        stepSizeInput.text = formatValue(stepSize)
        // Re-snap current values to new step size
        firstValue = rangeSlider.snapToStep(firstValue)
        secondValue = rangeSlider.snapToStep(secondValue)
        warningText.text = ""
        rangeChanged(firstValue, secondValue)
        updateQmlFile()
